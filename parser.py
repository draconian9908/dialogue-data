...
# links = []
# for let in alphas:
#     s = fp.parse('http://www.imsdb.com/feeds/alphabetical.php?letter=' + let)
#     for i in range(len(s['entries'])-1):
#         links.append(s['entries'][i]['link'])
...
def handle_movie (movie, browser):
    ...
    '''
    other_path = '/media/lhodges/B41C1C421C1C0252/Users/lhodges/Documents/ML/movie_scripts'
    # Unpack tuple
    ...
    list_tables = soup.findAll('table', "script-details")#[0].findAll('a')
    if list_tables[0]:
        list_links = list_tables[0].findAll('a')
        genre = []
        writer = []
        script = ''
        ...
        if script == '' or script[-5:] != '.html':
            path_to_directory = other_path + '/data/'
        ...
        # text to a file and include the movie in a csv file, with meta-information
            ...
            if len(soup.findAll('td', "scrtext"))!=1:
                error_file_name = other_path + '/data/scraping_error.csv'
            ...
            # Normal scraping:
                ...
                path_to_directory = other_path + '/data/texts/'
...
if __name__ == '__main__':
    ...
    # Create data/texts files
    if not os.path.exists('./data'):
        os.mkdir('./data')
        print('making ../data folder')
    if not os.path.exists('./data/texts'):
        os.mkdir('./data/texts')
        print('making ../data/texts folder')
    ...
    # in .csv format (in data folder)
    browser = webdriver.Chrome('/home/lhodges/Downloads/chromedriver')# Need to change this to work for individual's drivers
